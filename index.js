...
client.on("messageCreate", msg => {
	...
	if (db.data[msg.author.id].lastmsg === 0 ){
		db.data[msg.author.id].lastmsg = Date.now();
		db.write()
	}
	if (db.data[msg.author.id]) {
		if (Date.now() - db.data[msg.author.id].lastmsg >= 65000) {
			db.data[msg.author.id].xp += between(10,12)
			db.data[msg.author.id].lastmsg = Date.now();
			if (db.data[msg.author.id].xp > (db.data[msg.author.id].Levels * 112) && db.data[msg.author.id].xp < ((db.data[msg.author.id].Levels + 1) * 112)) {
				db.data[msg.author.id].Levels += 1
				msg.channel.send("<@" + msg.author.id + ">" + ", you advanced to level " + db.data[msg.author.id].Levels + ".")
